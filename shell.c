...
builtin_command builtins[NUM_BUILTINS] = {
    ...
    {"mkdir", shell_make_dir,
     "To create a new directory Usage: mkdir <directory name>"}};

void handle_sigint(int sig) {
  printf("\nCaught SIGINT (Ctrl+C). Type 'exit' or 'bye' to quit.\n");
...
int shell_exit_command(char **args) { return 0; }
...
int shell_launch(char **args) {
  ...
  if (pid == 0) {
    if (execvp(args[0], args) ==
        -1) // to make it such that external commands work if they dont exist in
            // the builtin commands of apksh
    {
...
int shell_roll_wrapper(char **args) { return shell_roll(); }
